    ...
    if (maxMatronId.length > 0) {
        ...
        for (let i = 0; i < maxMatronId.length; i++) {
            ...
            axios_1.default({
                ...
                .then((res) => {
                console.log("generation from above: " + generation);
                console.log("generation from eth: " + res.generation);
                console.log("created at from above " + birthTime);
                console.log("birth from here: " + res.birthTime);
...
function searchBlocks(startBlock, endBlock) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let blockNumberIter = startBlock; blockNumberIter < endBlock;) {
            ...
            let increment = RANGE_SIZE - incrementDifference + start;
            ...
            if (start <= end && end < increment) {
                ...
                if (start < increment && increment < end) {
                    ...
                    yield queryInfura(start, increment - 1, false); // in order to avoid overlap with next call, subtract one here.
                }
                ...
                else if (end === increment) {
                    if (verbose)
                        console.log("looking between blocks with no writing " + increment + " and " + (end));
                    yield queryInfura(start, end, false);
                }
            }
