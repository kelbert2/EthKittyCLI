...
console.log('Path of file in parent dir:', require('path').resolve(__dirname, storage));
console.log('Path of file in parent dir:', require('path').resolve(__dirname, '../dummy_data.json'));
...
for (let blockNumberIter = options.s; blockNumberIter <= options.e;) {
    ...
    if (start < end && end < startIncrement) {
        ...
        if (endIncrement > startIncrement) {
            ...
            console.log("looking between " + startIncrement + " and " + (endIncrement - 1));
        ...
        if (start < endIncrement && endIncrement < end) {
            console.log("looking between " + endIncrement + " and " + (end - 1));
...
function queryInfura(start, end, write = true) {
    ...
    //     }
    let result;
    fs_1.default.readFile('./dummy_data.json', "utf8", function (err, data) {
        var _a, _b;
        if (err) {
            console.log("Error reading file " + storage + ": " + err);
        }
