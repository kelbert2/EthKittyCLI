...
function searchBlocks(startBlock, endBlock) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let blockNumberIter = startBlock; blockNumberIter < endBlock;) {
            ...
            if (verbose)
                console.log((start - difference) + " - " + (startIncrement - difference) + " - " + (end - difference));
            ...
            if (start < end && end < startIncrement) {
                ...
                if (start < startIncrement && startIncrement < end) {
                    if (verbose)
                        console.log("looking between blocks " + start + " and " + (startIncrement - 1));
                    yield checkFile(start, startIncrement);
                }
                // Stats from startIncrement to endIncrement
                if (end > startIncrement) {
                    if (verbose)
                        console.log("looking between blocks " + startIncrement + " and " + (end - 1));
                    yield checkFile(startIncrement, end);
                }
            }
            blockNumberIter = end;
        }
